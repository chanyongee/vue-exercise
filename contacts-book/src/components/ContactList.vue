<template>
    <div>
        <p class="addnew">
            <button class="btn btn-primary" @click="onClickAddButton()">새로운 연락처 추가하기</button>
        </p>
        <div id="example">
            <table id="list" class="table table-striped table-bordered table-hover">
                ...
                <tbody id="contacts">
                    <tr v-for="(contact, idx) in contactlist.contacts" :key="idx">
                        <td>{{contact.name}}</td>
                        <td>{{contact.tel}}</td>
                        <td>{{contact.address}}</td>
                        <td><img :src="contact.photo" class="thumbnail" @click="onClickPhoto(contact.no)"></td>
                        <td>
                            <button class="btn btn-primary" @click="onClickUpdateButton(contact.no)">편집</button>
                            <button class="btn btn-primary" @click="onClickDeleteButton(contact.no)">삭제</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    methods: {
        onClickAddButton() {
            eventBus.$emit('onClickAddButton');
        },
        onClickUpdateButton(idx) {
            eventBus.$emit('onClickUpdateButton', idx);
        },
        onClickDeleteButton(idx) {
            eventBus.$emit('onClickDeleteButton', idx);
        },
        onClickPhoto(idx) {
            eventBus.$emit('onClickPhoto', idx);
        }
    }
}
</script>
